import { Team, Progress, Station } from '../models';
import { Op } from 'sequelize';
import { ILeaderboardEntry, IEventStatistics } from '../types';
...
class LeaderboardService {
   ...
   */
  async getTeamRank(teamId: number): Promise<{ rank: number | null; total: number; [key: string]: any }> {
    try {
   ...
   */
  async getDetailedLeaderboard(): Promise<ILeaderboardEntry[]> {
    try {
      const leaderboard = await this.getLeaderboard();
      ...
      const detailed = await Promise.all(leaderboard.map(async (entry) => {
        ...
        entry.stationProgress = progress.map(p => {
          const station = (p as any).station;
          return {
    ...
    try {
      ...
      if (finishedTeams.length > 0) {
        const totalTime = finishedTeams.reduce((sum, team) => {
          const duration = new Date(team.completedAt!).getTime() - new Date(team.startedAt!).getTime();
          return sum + duration;
        }, 0);

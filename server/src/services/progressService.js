...
class ProgressService {
   ...
   */
  async initializeTeamProgress(teamId) {
    try {
    ...
    try {
      const progress = await Progress.findAll({
        ...
        include: [{
          model: Station,
          as: 'station',
          attributes: ['id', 'stationNumber', 'title', 'description', 'location', 'challengeType', 'points', 'isActive']
        }],
   ...
   */
  async canAccessStation(teamId, stationId) {
    try {
   ...
   */
  async startStation(teamId, stationId) {
    try {
   ...
   */
  async completeStation(teamId, stationId, submissionData = {}, pointsEarned = 0) {
    try {
      const team = await Team.findByPk(teamId);
      const station = await Station.findByPk(stationId);
   ...
   */
  async submitForVerification(teamId, stationId, submissionData = {}) {
    try {
      const accessCheck = await this.canAccessStation(teamId, stationId);
      if (!accessCheck.allowed) {
        return { success: false, message: accessCheck.reason };
   ...
   */
  async verifySubmission(progressId, approved, pointsAwarded, adminId) {
    try {
   ...
   */
  async getAccessibleStations(teamId) {
    try {
      const team = await Team.findByPk(teamId);
   ...
   */
  async calculateRiddlePoints(stationId, attemptsCount, hintsUsed) {
    try {
      const station = await Station.findByPk(stationId);
      ...
      const minPoints = Math.floor(basePoints * (settings.minPointsPercentage / 100));
   ...
   */
  async manualComplete(teamId, stationId, points, adminId, reason = '') {
    try {
      ...
      const result = await this.completeStation(teamId, stationId, submissionData, points);
      ...
      const pending = await Progress.findAll({
        ...
        include: [
          ...
          },
          {
            model: Station,
            as: 'station',
            attributes: ['id', 'stationNumber', 'title', 'challengeType', 'points']
          }
        ],
        order: [['createdAt', 'ASC']]
      });

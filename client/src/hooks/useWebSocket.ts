...
export const useWebSocket = (teamCode: string | undefined, onProgressUpdate?: (data: any) => void) => {
  useEffect(() => {
    if (!teamCode) return
...
export const useLeaderboardWebSocket = (onLeaderboardUpdate?: (leaderboard: any[]) => void) => {
  useEffect(() => {
    const socket = websocket.connect()

...
import { Loader2, Plus, Search, RefreshCw, MapPin, Trophy, Camera, FileText } from 'lucide-react'
...
function StationsManagement() {
  ...
  const triviaCount = stationsArray.filter((s) => s.challengeType === 'trivia').length
  const riddleCount = stationsArray.filter((s) => s.challengeType === 'riddle').length
  const photoCount = stationsArray.filter((s) => s.challengeType === 'photo').length
  const physicalCount = stationsArray.filter((s) => s.challengeType === 'physical').length
  const totalPoints = stationsArray.reduce((sum, s) => sum + (s.points || s.pointsAvailable || 0), 0)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-3xl font-cinzel text-gold mb-2">Station Management</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4">
        <Card className="bg-blue-deep/90 border-2 border-gold/30 p-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-papyrus/70 text-sm font-lato">Total Stations</p>
        ...
        <Card className="bg-blue-deep/90 border-2 border-gold/30 p-4">
          <div>
            ...
            <p className="text-2xl font-cinzel text-purple-400">{riddleCount}</p>
          </div>
        </Card>
        <Card className="bg-blue-deep/90 border-2 border-gold/30 p-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-cinzel text-green-400">{photoCount}</p>
            </div>
        ...
        <Card className="bg-blue-deep/90 border-2 border-gold/30 p-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-cinzel text-orange-400">{physicalCount}</p>
            </div>
        ...
        <AlertDialogContent className="bg-blue-deep border-2 border-gold/30">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-papyrus/70 font-lato">
              Are you sure you want to delete station "{stationToDelete?.title}"? This action
              cannot be undone and will affect all team progress at this station.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
